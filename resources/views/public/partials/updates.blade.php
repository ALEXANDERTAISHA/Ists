    ...
    <div class="upd-container">
        ...
        <div class="upd-grid-two">
            ...
            <article class="upd-show-card upd-show-card--video">
                ...
                <h3 class="upd-card-title">{{ $videoUpdate->title ?? 'Último video institucional' }}</h3>
                @if(!empty($videoUpdate->description))
                    <p class="upd-card-desc">{{ \Illuminate\Support\Str::limit(strip_tags(html_entity_decode($videoUpdate->description)), 135) }}</p>
                @endif

                <div class="upd-media-box upd-media-box--video">
                    @if(!empty($videoUpdate->video_url))
                        <iframe src="{{ $videoUpdate->video_url }}" allowfullscreen loading="lazy" title="{{ $videoUpdate->title }}"></iframe>
                    @elseif(!empty($videoUpdate->video_path))
                        <video controls preload="metadata">
                            <source src="{{ asset('storage/' . $videoUpdate->video_path) }}" type="video/mp4">
                        </video>
                    @elseif(!empty($videoUpdate->image_path))
                        <img src="{{ asset('storage/' . $videoUpdate->image_path) }}" alt="{{ $videoUpdate->title }}" loading="lazy">
                    @else
                ...
                <div class="upd-meta-row">
                    <span>{{ optional($videoUpdate->date)->format('d M, Y') }}</span>
                    @if(!empty($videoUpdate->link_url))
                    <a href="{{ $videoUpdate->link_url }}" target="_blank" rel="noopener">Ver más</a>
                    @endif
                </div>
            </article>
            @endif
            ...
            <article class="upd-show-card upd-show-card--image">
                ...
                <h3 class="upd-card-title">{{ $imageUpdate->title ?? 'Última fotografía institucional' }}</h3>
                @if(!empty($imageUpdate->description))
                    <p class="upd-card-desc">{{ \Illuminate\Support\Str::limit(strip_tags(html_entity_decode($imageUpdate->description)), 135) }}</p>
                @endif

                <div class="upd-media-box upd-media-box--image">
                    @if(!empty($imageUpdate->image_path))
                        <img src="{{ asset('storage/' . $imageUpdate->image_path) }}" alt="{{ $imageUpdate->title }}" loading="lazy">
                    @elseif(!empty($imageUpdate->video_url))
                        <iframe src="{{ $imageUpdate->video_url }}" allowfullscreen loading="lazy" title="{{ $imageUpdate->title }}"></iframe>
                    @elseif(!empty($imageUpdate->video_path))
                        <video controls preload="metadata">
                            <source src="{{ asset('storage/' . $imageUpdate->video_path) }}" type="video/mp4">
                        </video>
                    @else
                ...
                <div class="upd-meta-row">
                    <span>{{ optional($imageUpdate->date)->format('d M, Y') }}</span>
                    @if(!empty($imageUpdate->link_url))
                    <a href="{{ $imageUpdate->link_url }}" target="_blank" rel="noopener">Ver más</a>
                    @endif
                </div>
            </article>
            @endif
