    ...
    <div class="container" style="position:relative; z-index:1;">
        ...
        <section class="academic-block">
            <div class="academic-block-head">
                <h2>Carreras</h2>
                <a href="{{ url('/carreras') }}" class="academic-inline-link">Ver catálogo completo</a>
            </div>
                ...
                @forelse($careers as $career)
                    ...
                    <article class="academic-card">
                        ...
                        <div class="academic-card-body">
                            <h3>{{ $career->name }}</h3>
                            <p>{{ \Illuminate\Support\Str::limit(strip_tags($career->description ?: 'Conoce el contenido, enfoque y perfil de egreso de esta carrera.'), 120) }}</p>
                            <a href="{{ route('career.show', $career->slug) }}" class="academic-card-link">Ver carrera</a>
                        </div>
                    </article>
                @empty
            ...
            <div class="academic-grid academic-grid--courses">
                @forelse($courses as $course)
                    ...
                    <article class="academic-card academic-card--course">
                        ...
                        <div class="academic-card-body">
                            <h3>{{ $course->title }}</h3>
                            <p>{{ \Illuminate\Support\Str::limit(strip_tags($course->description ?: 'Programa de formación continua disponible para nuestra comunidad.'), 120) }}</p>
                            <a href="{{ $courseUrl }}" class="academic-card-link" @if($course->url) target="_blank" rel="noopener" @endif>
                                Ver curso
                            </a>
                        </div>
                    </article>
