                ...
                <div class="carousel-inner" style="width:100%; height:100%;">
                    @foreach ($heroSlides as $index => $slide)
                        ...
                        <div class="carousel-item {{ $index === 0 ? 'active' : '' }}"
                             style="position:absolute; inset:0; width:100%; height:100%; background:url('{{ $slide->image_url }}') center center / cover no-repeat;">
                                {{-- Gradient overlays --}}
                                <div class="hero-overlay-gradient"></div>
                                {{-- Content --}}
                                <div class="hero-caption-wrap">
                                    <div class="hero-caption-inner {{ $heroGlassMode === 'dark' ? 'hero-caption-inner--dark' : 'hero-caption-inner--light' }}">
                                        ...
                                        @endif
                                        <div class="hero-ctas">
                                            <a href="{{ url('/carreras') }}" class="hero-btn-primary">
                                                Ver carreras
                                            ...
                                            <a href="{{ url('/noticias') }}" class="hero-btn-secondary">Noticias</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @foreach($careers as $career)
                        ...
                        <a href="{{ route('career.show', $career->slug) }}" class="hc-card" style="--hc-grad:{{ $meta['grad'] }};--hc-glow:{{ $meta['glow'] }};">
                            ...
                            <div class="hc-card-visual">
                                ...
                                {{-- Tag --}}
                                <span class="hc-card-tag" style="color:{{ $meta['tag_color'] }};background:{{ $meta['tag_bg'] }};">{{ $meta['tag'] }}</span>
                            </div>

                            {{-- Body --}}
                            <div class="hc-card-body">
                                <h3 class="hc-card-title">{{ $career->name }}</h3>
                                @if($career->description)
                                    <p class="hc-card-desc">{{ Str::limit(strip_tags($career->description), 90) }}</p>
                                @endif
        ...
        <section class="gac-section">
            <div class="container" style="position:relative;z-index:1;">
                ...
                <div class="gac-layout">
                    ...
                    {{-- Featured card --}}
                    <a href="{{ $getGacetaUrl($featuredItem) }}" class="gac-featured">
                        <div class="gac-featured-img-wrap">
                            @php $fImg = $getGacetaImg($featuredItem); @endphp
                            @if($fImg)
                                <img src="{{ $fImg }}" alt="{{ $featuredItem->title }}" class="gac-featured-img">
                            @else
                        ...
                        <div class="gac-featured-body">
                            <div class="gac-featured-meta-row">
                                <span class="gac-tag {{ $getGacetaTagClass($featuredItem) }}">{{ $getGacetaTag($featuredItem) }}</span>
                                @php $fd = $getGacetaDate($featuredItem); @endphp
                                @if($fd)<span class="gac-featured-date">{{ $fd }}</span>@endif
                            </div>
                            <h3 class="gac-featured-title">{{ $featuredItem->title }}</h3>
                            <p class="gac-featured-excerpt">{{ \Illuminate\Support\Str::limit(strip_tags(html_entity_decode($featuredItem->summary)), 170) }}</p>
                    ...
                    {{-- Side cards --}}
                    <div class="gac-side">
                        @foreach($restItems as $n)
                        @php $sImg = $getGacetaImg($n); @endphp
                        <a href="{{ $getGacetaUrl($n) }}" class="gac-card">
                            <div class="gac-card-img-wrap">
                                @if($sImg)
                                    <img src="{{ $sImg }}" alt="{{ $n->title }}" class="gac-card-img">
                                @else
                            ...
                            <div class="gac-card-body">
                                <div class="gac-card-meta">
                                    ...
                                    @php $sd = $getGacetaDate($n); @endphp
                                    @if($sd)<span class="gac-card-date">{{ $sd }}</span>@endif
                                </div>
                                <h4 class="gac-card-title">{{ $n->title }}</h4>
                                <p class="gac-card-excerpt">{{ \Illuminate\Support\Str::limit(strip_tags(html_entity_decode($n->summary)), 90) }}</p>
                ...
                <div class="gac-actions">
                    <a href="{{ url('/noticias') }}" class="gac-btn-all">
    ...
    <div id="social-widget"
        ...
        @foreach($socialLinks as $link)
            <a href="{{ $link->url }}" target="_blank" rel="noopener" title="{{ ucfirst($link->name) }}"
               class="sw-link"
               style="--sw-bg:{{ $link->bg_color }}; --sw-glow: rgba(0,0,0,0.25);">
                <div class="sw-link-inner">
                    {!! $link->icon_svg !!}
                </div>
                <span class="sw-tooltip">{{ ucfirst($link->name) }}</span>
            </a>
        @endforeach
