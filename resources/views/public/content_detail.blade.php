...
<head>
    ...
    <meta name="description" content="{{ is_array($content) ? ($content['description'] ?? '') : ($content->description ?? '') }}">
    <title>{{ is_array($content) ? ($content['title'] ?? 'Contenido - ISTS') : ($content->title ?? 'Contenido - ISTS') }}</title>
...
<body>
    ...
    <section>
        <div class="container">
            <h1 class="detail-title">{{ $contentData['title'] ?? 'Contenido' }}</h1>
        </div>
    </section>
    ...
    <section class="about-content">
        <div class="container">
            @if (!$isTransparency)
                <div class="content-intro-grid{{ $isOrganigrama ? ' content-intro-grid--organigrama' : '' }}">
                    <div class="content-image-box">
                        <img src="{{ $imgSrc }}" alt="{{ $contentData['title'] ?? 'Imagen de contenido' }}">
                    </div>
                    @if(!$isOrganigrama || !empty(trim(strip_tags($contentData['content'] ?? ''))))
                ...
                @if ($contentFileLink)
                    <div class="content-file-download">
                        <a href="{{ $contentFileLink }}" target="_blank" class="pdf-pro-link">Ver PDF</a>
                    </div>
                @endif
            @else
                @if(!$isRendicion && !empty($contentData['description']))
                    <p class="transparency-intro">{{ $contentData['description'] }}</p>
                @endif

                @if ($contentFileLink)
                    <div class="content-file-download" style="margin-bottom: 1.25rem;">
                        <a href="{{ $contentFileLink }}" target="_blank" class="pdf-pro-link">Abrir documento principal</a>
                    </div>
                @endif
                    ...
                    <div class="subreglamentos-list">
                        ...
                        <div class="transparency-level-grid">
                            @foreach($children as $child)
                                ...
                                @endphp
                                <a href="{{ $childHref }}" target="{{ $childTarget }}" class="transparency-level-card">
                                    <div class="level-card-head">
                                        <h3 class="level-card-title">{{ $child['title'] }}</h3>
                                    </div>
                                    <div class="level-card-body">
                                        @if(!empty($child['description']))
                                            <p class="level-card-description">{{ \Illuminate\Support\Str::limit(strip_tags($child['description']), 120) }}</p>
                                        @endif
                                    </div>
                                </a>
                            @endforeach
