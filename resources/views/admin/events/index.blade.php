...
<div class="card admin-page-card admin-page-card--events" style="border-radius: 18px; box-shadow: 0 2px 16px rgba(0,158,96,0.10); margin-top: 2.5rem;">
    <div class="card-body p-5">
        <div class="mb-4 d-flex flex-column flex-md-row align-items-center justify-content-between admin-page-header" style="gap:1.2rem;">
            ...
            <a href="{{ route('admin.events.create') }}" class="btn-new"><i class="bi bi-plus-lg"></i> Crear evento</a>
        </div>
        @if(session('success'))
                ...
                <tbody>
                    @forelse($events as $event)
                        <tr style="background: #fff;">
                            ...
                            <td>{{ $event->place }}</td>
                            <td>
                                <span class="badge" style="background:{{ $event->status==='published' ? '#009e60' : '#f9d423' }};color:#fff; font-weight:600; border-radius:6px; padding:0.4em 1em; font-size:0.98em;">
                                    {{ $event->status==='published' ? 'Publicado' : 'Borrador' }}
                                </span>
                            </td>
                            <td>
                                @if($event->image_path)
                                    <img src="{{ asset('storage/' . $event->image_path) }}" alt="Imagen" style="max-width:60px;max-height:40px; border-radius:6px; box-shadow:0 2px 8px rgba(0,158,96,0.10);">
                                @else
                                    <span class="text-muted">Sin imagen</span>
                                @endif
                            </td>
                                                        <td class="admin-actions-cell" style="display:flex; gap:0.5em;">
                                  <a href="{{ route('admin.events.edit', $event) }}" class="btn btn-edit btn-sm"><i class="bi bi-pencil-square"></i> Editar</a>
